...
##split arrays out into seperate ages, apply cut offs and then stick them all back together again via rownames...
...
xpd11 <- genefilter(E11,filterfun(function(x){sum(x >= cut.off) >= min.array}))
E11 <- E11[xpd11,]

varying11 <- genefilter(E11,filterfun(function(x){sd(x)>= sd.threshold}))
E11 <- E11[varying11,]

xpd12 <- genefilter(E12,filterfun(function(x){sum(x >= cut.off) >= min.array}))
E12 <- E12[xpd12,]

varying12 <- genefilter(E12,filterfun(function(x){sd(x)>= sd.threshold}))
E12 <- E12[varying12,]

xpd13 <- genefilter(E13,filterfun(function(x){sum(x >= cut.off) >= min.array}))
E13 <- E13[xpd13,]

varying13 <- genefilter(E13,filterfun(function(x){sd(x)>= sd.threshold}))
E13 <- E13[varying13,]

E.rownames <- c(rownames(E11),rownames(E12),rownames(E13))
E.rownames.o <- E.rownames[order(E.rownames,decreasing=TRUE)]
E.rownames.o.d <- E.rownames[!duplicated(E.rownames.o)]
...
write.csv(results, file="results/results_timecourse.csv", row.names=F)
save(results, file="results/results_timecourse.RData")
